		...
		if (file && typeof file === 'object' && 'metadata' in file) {
				...
				if (blog.startsWith(`/series/${coverFolder}/`)) {
					...
					if (subPost && typeof subPost === 'object' && 'metadata' in subPost) {
						...
						const subPostData = {
							...
							coverDescription: metadata.description,
						...
						if (!subPostMetadata.draft) {
							subPosts.push(subPostData);
						}
					}
				}
			}
			...
			if (!metadata.draft) {
				seriesPosts.push({
					...
					// add totalParts to the subPosts array
					subPosts: subPosts.map((subPost) => ({ ...subPost, totalParts })),
