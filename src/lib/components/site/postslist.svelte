<script lang="ts">
	...
	$: seriesSlug = $page.url.pathname.split('/').pop() || '';

	$: isSeriesPost = pattern.test(seriesSlug);

	let activeSeriesPost: SeriesPost | undefined;

	$: {
		if (isSeriesPost) {
			// find the slug in subPosts
			activeSeriesPost = seriesPosts.find((seriesPost) => {
			...
			<Accordion.Root class="flex flex-col gap-1" value={activeSeriesPost?.coverFolder}>
				{#each seriesPosts as seriesPost}
					<Accordion.Item value={seriesPost.coverFolder} class="border-none">
						<Accordion.Trigger
							class={cn(
								'rounded-lg transition-colors duration-300 hover:bg-gray-200 [&>*]:transition-colors [&>*]:duration-300',
								activeSeriesPost?.coverFolder === seriesPost.coverFolder
									? 'bg-gray-200'
									: $seriesPostCoverFolder === seriesPost.coverFolder
										? 'bg-gray-200'
										: ''
							)}
							on:click={() => {
